...
class AdminHomeFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val user = view.findViewById<TextView>(R.id.admin_username)
        user.text = "Halo, admin!"

        val kelola = view.findViewById<Button>(R.id.btn_kelola)
        kelola.setOnClickListener{
            val fragment: Fragment = AdminQuizFragment()
            val transaction = fragmentManager?.beginTransaction()
            transaction?.replace(R.id.admin_frame_layout,fragment)?.commit()
        }

        val total = view.findViewById<TextView>(R.id.total_pemain)
        ...
        database.addListenerForSingleValueEvent(object : ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                total.text = snapshot.childrenCount.toString()
            }
